...
function renderConsistencyResults(result, reqs) {
  ...
  wrap.innerHTML = `
    ...
    ${result.warnings.length ? `
      ...
      ${result.warnings.map(w => `<div style="background:var(--ambbg);border:1px solid rgba(251,191,36,.2);border-radius:var(--r);padding:10px 12px;margin-bottom:6px;font-size:12px">
...
function showACSuggestions(sugg, titleInputId, descInputId) {
  ...
  div.innerHTML = `
    ...
    ${(sugg.suggestions||[]).map(s => `<div class="ac-suggestion-item" onclick="applyACSuggestion('${titleInputId}','${descInputId}','${esc(s).replace(/'/g,"\\'")}','${esc(sugg.description||'').replace(/'/g,"\\'")}','${esc(sugg.category||'Funktional')}')"
