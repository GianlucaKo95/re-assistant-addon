    ...
    wrap.innerHTML = `
      <div class="kanban-board">
        ${data.states.map(state => `
          <div class="kanban-col" data-status="${state.id}">
            <div class="kanban-col-head" style="border-top:3px solid ${state.color}">
              ...
              <span class="kanban-count" style="background:${state.color}22;color:${state.color};
                ...
                ${(data.board[state.id] || []).length}
              </span>
            </div>
            <div class="kanban-cards" data-status="${state.id}">
              ${(data.board[state.id] || []).map(req => kanbanCard(req, state.color)).join('')}
              ${!(data.board[state.id] || []).length
                ? '<div style="padding:20px;text-align:center;color:var(--t3);font-size:12px">Keine Anforderungen</div>'
                : ''}
            </div>
          </div>`).join('')}
      </div>`;
