...
function openStoryDetail(node) {
  ...
  openModal(`📖 ${esc(story.title)}`, `
    ...
    ${tests.length ? tests.map(t => `
      <div style="background:var(--s2);border-radius:var(--r);padding:8px 10px;margin-bottom:6px;font-size:12px">
      ...
      </div>`).join('') : '<div style="color:var(--t3);font-size:12px">Keine Testfälle</div>'}
