...
async function devAnalyzeSource(reqId) {
  ...
  try {
    ...
    try {
      ...
      // Bei max_tokens abgeschnitten: vollständige Dateien/Schritte retten
      const affected_files = extractJsonObjects(res.text, 'affected_files');
      const steps          = extractJsonObjects(res.text, 'steps');
      if (affected_files.length || steps.length) {
        analysis = { summary: 'Antwort unvollständig (Token-Limit) — evtl. nicht alle Schritte erkannt.',
          affected_files, steps, tests: '', risks: '', estimated_effort: '', notes: '' };
        toast('⚠ Antwort unvollständig — Plan evtl. nicht vollständig');
      } else {
        toast('❌ Parsing-Fehler — KI-Antwort war kein valides JSON' + (res.truncated ? ' (Längenbegrenzung)' : ''));
...
function renderCommentThread(r) {
  ...
  return `<div class="comment-thread">${(r.comments || []).map(c => `<div class="comment">
